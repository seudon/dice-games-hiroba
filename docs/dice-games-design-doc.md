...
実物のサイコロで遊ぶ（道具・人数・印刷ボタン）
詳しいルールと戦略（<details>で折りたたみ）
```

クイックルールをゲームの下に置いているのは、スマホでサイコロと操作ボタンを
1画面に収めるため。「まず触ってみて、分からなければすぐ下を読む」という順序を想定している。

スマホ（375×667）での実測では、サイコロは全7ゲームが1画面内に入り、
操作ボタンまで収まるのは5ゲーム。残る2つ（サイコロ早押し計算、丁半博打）は
設定UIやベット選択の要素数が多く、構造上どうしても縦に伸びる。
