...
6. **開発サーバー再起動（最重要）** - ファイル修正後は**必ず**開発サーバーを再起動する
   - ⚠️ **Astroは自動リロードが効かないケースが多い** - 特に以下の場合は必須：
     - .astroファイルの修正（リンク、レイアウト、コンポーネント）
     - スタイル変更（scoped style、CSS）
     - 設定ファイル変更（astro.config.mjs、tsconfig.json）
     - Content Collections関連ファイルの変更
   - **再起動方法**: 開発サーバーを停止（Ctrl+C）してから `npm run dev` を実行
   - 変更が反映されない場合は、まず再起動を試すこと
